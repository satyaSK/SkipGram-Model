...
* Along with the skip-gram model, I produced these sweet vector embeddings with tensorboard to explore the data we are dealing with. What you see, are the word vector embeddings.
* Notice how they have begun to form clusters in 3-D space, with semantically similar words(occuring in the same context) coming closer together and dissimilar words going far away from each other. 
* The tags for these words can be seen in tensorboard, but I haven't incuded these in this picture to keep the visualization clean.
...
Observe how the loss plummets and saturates when it reaches 70k iteration mark. I have used the ```tf.train.GradientDescentOptimizer``` to optimize this loss function, and smoothened out the results a little, for observing the trend.
