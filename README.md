# Skip-Gram Model 
The above code implements the Skip-gram model from [this paper](https://arxiv.org/pdf/1310.4546.pdf). The main idea is to form word vector representations without losing the semantic information and relationships amongst various words. This is achieved by mapping the words to multi-dimensional space, and an observation is that similarities between words seem to be encoded in the vector difference between the words. Also I have included a word embedding visualization to visualize the output of this project.
...
Along with the skip-gram model, I produced these sweet vector embeddings with tensorboard to explore the data we are dealing with. What you see, are the word vector embeddings. Notice how they have begun to form clusters in 3-D space, with semantically similar words coming closer together and dissimilar words going away from each other. The tags for these words can be seen in tensorboard, but I haven't incuded these in this picture to keep the visualization clean.

# What are word Embeddings bro?
* Put in simple words, word embedding is a parameterized function whaich maps words in some given language, to higher dimensional vectors. And this fucntion is defined by the ```embedding matrix```.
...
* Converting the numbers into plain integers to pass them for training is of no use for 2 reasons
	* The integer associated with each word will vary in magnitude(in a random way).
	* Choosing single integers for indivisual word would not capture the relationships amongst words having similar meaning.
...
* So finally, we arrive at word vector embeddings. These vector embeddings are multi-dimensional representations of indivisual words. To make it less scary, word embeddings are simple statistical representations of words, in which a single word can be represented by a single vector, for example, a vector(or embedding) of size 128(aka 128-D vector).
...
* So we eventually want to train this vector representation aka ```embedding_matrix``` of words to represent words and relationships amongst words more accurately, such that words meaning the same thing or occuring in the same context can be found closer to each other in multi-dimensional space.
...
* There are 2 primary models which help us achieve this target:
...
* Here's an example to explain this better. Consider the sentence "I love to eat food", so for this sentence, given the center word is "love", we can say that the words "I", "to", "eat" and "food" are context words, which occur in the same "context" as "love".
...
* Notice that every single word in a corpus of text a text will have, not one, but two vector representations.
	* Firstly, while a word is a center word.
	* Secondly, when the word is a context word.
...
* The ```most_common``` words are given unique integers ID's. Therefore, each unique word would be associated with an integer which would, in turn, be associated with it's vector embedding of size ```word_embedding_size```.
* The dictionary of word ID's and the words themselves are passed to the ```generate_pairs``` function which generates center-target word pairs within a window of size ```window_size```.
* Finally, a ```next-batch``` function is created to fetch the next batch to pass into our model. Next batch will fetch integers corresponding to unique words, and the "not so common" words are replaced by the token ```NAN```.
* Personally for me, the data preprocessing part took up most of the time, but now, once this heavy-lifting is done, its just a matter of creating the model!
...
Observe how the loss falls and saturates when it reaches 70k iterations. I have used the ```tf.train.GradientDescentOptimizer``` to optimize this loss function, and smoothened out the results for observing the trend.
...
* We perceive that the correct context word is being learnt, which is actually a product of the model being able to distinguish whether a given word is a good word or a bad word, for a given center word, i.e. the model asks the question "Can this target word occur in the context of the center word?"
...
* The words are higher dimensional vectors, so in order to understand them intuitively, how could humans visualize these?
...
* Though i'm not going to go deep into the detailed functioning of the algorithm, its a pretty sweet visualization tool built into tensorboard which could help in exploring the data and visualizing our efforts :D
